...
public struct ReceivedPacket: Sendable {
    ...
    public static func parse(_ datagram: [UInt8]) -> ReceivedPacket? {
        parse(datagram, expectedDestination: nil)
    }

    static func parse(_ datagram: [UInt8], expectedDestination: IPv4Endpoint?) -> ReceivedPacket? {
        guard !datagram.isEmpty else { return nil }
        if datagram[0] >> 4 == 4,
           let header = try? IPv4.parseHeader(datagram[...]),
           header.protocolNumber == 1 {
            guard let message = try? ICMPv4.parseMessage(datagram[header.headerLength...], expectedDestination: expectedDestination) else { return nil }
            return ReceivedPacket(message: message, timeToLive: header.timeToLive, source: header.source)
        }
        guard let message = try? ICMPv4.parseMessage(datagram[...], expectedDestination: expectedDestination) else { return nil }
