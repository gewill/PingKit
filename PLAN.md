...
- `PingConfiguration.sendDuration` 是从发送循环开始计算的可选单调窗口；默认 `nil` 不改变普通 Ping。窗口结束或挂起后恢复时，每次 `socket.send` 前核对截止，禁止新包；已发送探测继续接收回复／超时，最长到最后一次发送后的配置 `timeout`。长于窗口的采样间隔不能拖延收尾。
