...
> 状态：M1–M8 已完成，当前版本 0.7.0；#39 有限发送窗口开发中，尚未发布。更新日期：2026-09-24
...
- `PingConfiguration.sendDuration` 是从 socket 建立后首次实际发送机会开始计算的可选单调窗口；默认 `nil` 不改变普通 Ping。窗口结束或挂起后恢复时，每次 `socket.send` 前核对截止，禁止新包；已发送探测继续接收回复／超时，最长到最后一次发送后的配置 `timeout`。长于窗口的采样间隔不能拖延收尾。
- `stop()` 和消费任务取消仍立即关闭 socket，留下的未结算探测由调用者按未知处理。Pingman #89／#92 使用这个边界实现限时后台诊断；是否能在真机系统提示与低电量下连续运行，仍由 Pingman #87 验收。
